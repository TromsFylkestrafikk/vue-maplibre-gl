/*!
* @indoorequal/vue-maplibre-gl v9.0.1+rtm-0.1
...
import { AttributionControl as _, FullscreenControl as v, GeolocateControl as y, LngLat as b, LogoControl as x, Map as ee, Marker as S, NavigationControl as te, Popup as ne, ScaleControl as re } from "maplibre-gl";
...
}, oe = Symbol("map"), C = oe, w = Symbol("isLoaded"), se = Symbol("isInitialized"), T = se, E = Symbol("componentId"), D = Symbol("sourceId"), O = Symbol("sourceLayerRegistry"), k = Symbol("marker"), ce = /* @__PURE__ */ "error,load,idle,remove,render,resize,webglcontextlost,webglcontextrestored,dataloading,data,sourcedataloading,styledataloading,sourcedata,styledata,styleimagemissing,dataabort,sourcedataabort,boxzoomcancel,boxzoomstart,boxzoomend,touchcancel,touchmove,touchend,touchstart,click,contextmenu,dblclick,mousemove,mouseup,mousedown,mouseout,mouseover,movestart,move,moveend,zoomstart,zoom,zoomend,rotatestart,rotate,rotateend,dragstart,drag,dragend,pitchstart,pitch,pitchend,wheel,terrain,cooperativegestureprevented,projectiontransition,style.load,rollstart,roll,rollend".split(",");
function le(e, t, n, r) {
	return (i) => n.emit(r, {
		type: i.type,
		map: t,
...
var P = n({
	...
	emits: /* @__PURE__ */ "map:error,map:load,map:idle,map:remove,map:render,map:resize,map:webglcontextlost,map:webglcontextrestored,map:dataloading,map:data,map:sourcedataloading,map:styledataloading,map:sourcedata,map:styledata,map:styleimagemissing,map:dataabort,map:sourcedataabort,map:boxzoomcancel,map:boxzoomstart,map:boxzoomend,map:touchcancel,map:touchmove,map:touchend,map:touchstart,map:click,map:contextmenu,map:dblclick,map:mousemove,map:mouseup,map:mousedown,map:mouseout,map:mouseover,map:movestart,map:move,map:moveend,map:zoomstart,map:zoom,map:zoomend,map:rotatestart,map:rotate,map:rotateend,map:dragstart,map:drag,map:dragend,map:pitchstart,map:pitch,map:pitchend,map:wheel,map:terrain,map:cooperativegestureprevented,map:projectiontransition,map:style.load,map:rollstart,map:roll,map:rollend,update:center,update:zoom,update:pitch,update:bearing".split(","),
	slots: Object,
	setup(e, t) {
		let n = s(r()), a = m(), o = m(), f = p(!1), h = p(!1), _ = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), y = fe(n, o, e.mapKey);
		d(C, o), d(w, h), d(T, f), d(E, n.uid), d(D, ""), g(() => e.bearing, (e) => {
		...
		function b() {
			...
			if (o.value = s(new ee(r)), y.map = o.value, f.value = !0, v.set("load", () => (h.value = !0, y.isLoaded = !0)), o.value.on("load", v.get("load")), v.set("moveend", () => t.emit("update:center", o.value.getCenter())), o.value.on("moveend", v.get("moveend")), v.set("zoomend", () => t.emit("update:zoom", o.value.getZoom())), o.value.on("zoomend", v.get("zoomend")), v.set("pitchend", () => t.emit("update:pitch", o.value.getPitch())), o.value.on("pitchend", v.get("pitchend")), v.set("rotateend", () => t.emit("update:bearing", o.value.getBearing())), o.value.on("rotateend", v.get("rotateend")), n.vnode.props) {
				for (let e of ce) if (n.vnode.props["onMap:" + e]) {
					let r = `map:${e}`, i = le(n, o.value, t, r);
					_.set(e, i), o.value.on(e, i);
				}
			}
			o.value.getCanvas().addEventListener("webglcontextlost", S);
		}
		async function x() {
			y.isMounted = !1, y.isLoaded = !1, h.value = !1, o.value && (o.value.getCanvas().removeEventListener("webglcontextlost", S), f.value = !1, _.forEach((e, t) => {
				o.value.off(t, e);
			}), v.forEach((e, t) => {
				o.value.off(t, e);
			}), o.value.remove());
		...
		return u(b), l(x), t.expose({ map: o }), () => [i("div", {
			ref: a,
			style: {
				height: e.height,
				width: e.width
...
var L = n({
	...
	setup(e, { slots: t, emit: n }) {
		let r = a(C), o = m(), s = p(), c = p(!1), f = /* @__PURE__ */ new Map();
		function h(e, t) {
			let r = (r) => {
				t && t(), n(e, r);
			};
		...
		return d(k, o), u(() => {
			let i = { ...e };
			t.marker && (i.element = s.value), o.value = new S(i), o.value.setLngLat(e.coordinates).addTo(r.value), h("dragstart"), h("drag", () => {
	...
	setup(e, { slots: t, emit: n, expose: r }) {
		let o = a(C), s = a(k, void 0), c = p(), d = new ne(e);
...
function Ee(e, t, n) {
	let r = Te(e, t, n);
	r in X && (X[r].unsubscribe(), delete X[r]);
}
function De(e, t, n, r) {
	Ee(e, t, n), X[Te(e, t, n)] = r;
}
function Oe(e, t, n) {
	if (n.props) for (let r of J) {
		let i = "on" + r.charAt(0).toUpperCase() + r.substring(1);
		n.props[i] && De(e, t, r, e.on(r, t, n.props[i]));
	}
}
function ke(e, t, n) {
	if (n.props) for (let n of J) Ee(e, t, n);
}
//#endregion
//#region lib/composable/useDisposableLayer.ts
function Z(e, t) {
	let n = a(C), r = a(w), i = a(O);
	function o() {
		r.value && (t && ke(n.value, e, t.vnode), n.value.getLayer(e) && n.value.removeLayer(e));
	}
...
var Ae = n({
	...
	setup(e) {
		let n = a(C), r = a(w);
		return Z(e.layerId), g(() => e.layout, (t) => {
...
function Q(e, n) {
	...
	let o = r(), s = a(C), c = a(w), l = a(E), u = K.getSourceRef(l, n.source || i);
	return Z(n.layerId, o), g(() => n.minzoom, () => s.value.setLayerZoomRange(n.layerId, n.minzoom || 0, n.maxzoom || 24)), g(() => n.maxzoom, () => s.value.setLayerZoomRange(n.layerId, n.minzoom || 0, n.maxzoom || 24)), g(() => n.layout, (e) => {
		...
		t && (r || r === void 0) && (s.value.addLayer(we(n.layerId, e, n, i), n.before || void 0), Oe(s.value, n.layerId, o.vnode));
...
var $ = n({
	...
	MglBackgroundLayer: () => Ae,
	MglCanvasSource: () => _e,
	MglCircleLayer: () => $,
	MglCustomControl: () => R,
	MglFillExtrusionLayer: () => Me,
	MglFillLayer: () => je,
	MglFullscreenControl: () => z,
	MglGeoJsonSource: () => ve,
	MglGeolocateControl: () => B,
	MglHeatmapLayer: () => Ne,
	MglHillshadeLayer: () => Pe,
	MglImage: () => ge,
	MglImageSource: () => ye,
	MglLineLayer: () => Fe,
	MglLogoControl: () => U,
...
//#endregion
export { L as MglAttributionControl, Ae as MglBackgroundLayer, _e as MglCanvasSource, $ as MglCircleLayer, R as MglCustomControl, Me as MglFillExtrusionLayer, je as MglFillLayer, z as MglFullscreenControl, ve as MglGeoJsonSource, B as MglGeolocateControl, Ne as MglHeatmapLayer, Pe as MglHillshadeLayer, ge as MglImage, ye as MglImageSource, Fe as MglLineLayer, U as MglLogoControl, P as MglMap, W as MglMarker, V as MglNavigationControl, he as MglPopup, xe as MglRasterDemSource, Ie as MglRasterLayer, be as MglRasterSource, H as MglScaleControl, Le as MglSymbolLayer, Se as MglVectorSource, Ce as MglVideoSource, A as Position, E as componentIdSymbol, ze as default, se as isInitialized, T as isInitializedSymbol, w as isLoadedSymbol, oe as map, C as mapSymbol, k as markerSymbol, D as sourceIdSymbol, O as sourceLayerRegistry, I as useControl, Z as useDisposableLayer, de as useMap, F as usePositionWatcher, q as useSource };
